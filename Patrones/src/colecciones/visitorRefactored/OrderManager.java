...
public class OrderManager extends JFrame {
	...
	public OrderManager() {
		...
		cmbOrderType.addItem(OrderManager.BRAZILIAN_ORDER); // new order Type
		...
		lblAdditionalSH = new JLabel("Additional S & H(Overseas & Brazilian Orders Only):");
...
class ButtonHandler implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getActionCommand().equals(OrderManager.GET_TOTAL)) {
			...
			Vector orderList = visitor.getOrderObjList();

			BarCodeVisitor bcVisitor = new BarCodeVisitor();
			Double totalAmount = new Double(0);
			for (Object object : orderList) {
				Order order = (Order) object;

				order.accept(visitor);
				order.accept(bcVisitor);
				
				totalAmount = totalAmount + visitor.getOrderTotal();
			}
			// accept the visitor instance
			totalResult = totalAmount.toString();
