...
public class CommandProcesor implements Serializable {
	...
	class Memento implements Serializable {
		...
		private Vector<CommandInterface> getCommandList() {
			return commandList;
		}

		private void setCommand(Vector<CommandInterface> commandLst) {
			this.commandList = commandLst;
		}

		private Memento(Vector<CommandInterface> commandLst) {
			this.commandList = commandLst;
		}
	...
	public void setMemento(Memento memento) {
		if (memento != null) {
			commandList = memento.getCommandList();
			}else {
				getCommandsFromFile();
			}
	}
